...
# Returns false if the board has any zeros
def checkFull(board):
    for lst in board:
        for l in lst:
            if l == 0:
...
class Sudoku:
    ...
    def complete(self):
        ...
        for x in range(9):
            for val in values:
                if val not in self.row(x):
                    return False

        for x in range(9):
            for val in values:
                if val not in self.col(x):
                    return False

        for x in range(9):
            for val in values:
                if val not in self.box(x):
                    return False
    ...
    def variableDomain(self, r, c):
        ...
        for val in self.col(c):
            if val in domain:
                domain.remove(val)

        for val in self.box(self.box_id(r,c)):
            if val in domain:
                domain.remove(val)
    ...
    def updateFactor(self, factor_type, i):
        ...
        # if factorRemaining isn't existing, create it.
        """
        if (factor_type, i) not in self.factorRemaining:
            self.factorRemaining[(factor_type, i)] = [0,1,2,3,4,5,6,7,8]
            self.factorNumConflicts[(factor_type, i)] = 0

        if factor_type == BOX:
            violations = crossOff(self.factorRemaining[(factor_type, i)], self.box(i))
        ...
        if factor_type == BOX:
            violations = crossOff(values, self.box(i))
            self.factorNumConflicts[(factor_type, i)] = violations
            self.factorRemaining[(factor_type, i)] = values

        if factor_type == ROW:
            violations = crossOff(values, self.row(i))
            self.factorNumConflicts[(factor_type, i)] = violations
            self.factorRemaining[(factor_type, i)] = values

        if factor_type == COL:
            violations = crossOff(values, self.col(i))
            self.factorNumConflicts[(factor_type, i)] = violations
            self.factorRemaining[(factor_type, i)] = values
    ...
    def updateVariableFactors(self, variable):
        ...
        """
        r,c = variable
        boxId = self.box_id(r,c)

        self.updateFactor(BOX, boxId)
    ...
    def getSuccessors(self):
        ...
        for val in self.variableDomain(r,c):
            successor.append(self.setVariable(r,c, val))
        ...
        for x in range(9):
            for y in range(9):
                if not self.board[x][y]: 
                    if not self.variableDomain(x,y):
                        return False
        return True
    ...
    def randomRestart(self):
        ...
        while not checkFull(self.board):
            r,c = self.firstEpsilonVariable()
            
            factors = deepcopy(self.factorRemaining[ROW,r])
            remove_values_from_list(factors, None)


            val = (random.choice(factors))
    ...
    def randomSwap(self):
        ...
        while boolean:
            ...
            r3 = random.choice(lst)
            r4 = random.choice(lst)

            # Now, we have 3 variables.
            if self.originalBoard[r1][r2] == 0:
                if self.originalBoard[r3][r4] == 0:
                    if (r1,r2) != (r3,r4):
                        return ((r1,r2),(r3,r4))
    ...
    def gradientDescent(self, variable1, variable2):
        ...
        """
        oldConflicts = self.numConflicts()
        self.modifySwap(variable1, variable2)

        newConflicts = self.numConflicts()
        """
        print oldConflicts, newConflicts
        """
        if newConflicts > oldConflicts or random.randrange(2) != 0:
            self.modifySwap(variable1, variable2)
            print oldConflicts, newConflicts
